...
$isEdit = $id > 0;
$page_title = $isEdit ? 'Edit Transport' : 'Add Transport';
$errors = [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        if ($isEdit) {
            $stmt = $conn->prepare("UPDATE transports SET name=?, description=?, image_url=?, default_price=? WHERE id=?");
            $stmt->bind_param('sssdi', $item['name'], $item['description'], $item['image_url'], $item['default_price'], $id);
        } else {
            $stmt = $conn->prepare("INSERT INTO transports (name, description, image_url, default_price) VALUES (?,?,?,?)");
            $stmt->bind_param('sssd', $item['name'], $item['description'], $item['image_url'], $item['default_price']);
        }
        $stmt->execute();
        header('Location: transports.php?saved=1');
        exit;
    }
} elseif ($isEdit) {
    ...
    if (!$found) { header('Location: transports.php'); exit; }
    $item = $found;
}
?>
...
<div class="admin-page-header">
    <div><h1><?= $isEdit ? 'EDIT TRANSPORT' : 'ADD TRANSPORT' ?></h1></div>
    <a href="transports.php" class="btn btn-outline">&larr; BACK TO TRANSPORTS</a>
</div>
...
<form method="POST">
    ...
    <div style="display:flex;gap:12px;">
        <button type="submit" class="btn btn-primary"><?= $isEdit ? 'SAVE CHANGES' : 'ADD TRANSPORT' ?></button>
        <a href="transports.php" class="btn btn-outline">CANCEL</a>
    </div>
</form>
